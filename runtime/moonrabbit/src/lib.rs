...
parameter_types! {
	pub Prefix: &'static [u8] = b"Pay AAAs to the kusama account:";
}
...
parameter_types! {
	// Minimum 100 bytes/AAA deposited (1 CENT/byte)
...
parameter_types! {
	...
	pub const SubmissionDeposit: Balance = 3 * GRAND; // ~ 10 AAA
	pub const MinContribution: Balance = 3_000 * CENTS; // ~ .1 AAA
...
parameter_types! {
	...
	pub const EndingPeriod: BlockNumber = 10 * MINUTES;
...
parameter_types! {
	/// The location of the AAA token, from the context of this chain. Since this token is native to this
	/// chain, we make it synonymous with it and thus it is the `Null` location, which means "equivalent to
	/// the context".
	pub const AaaLocation: MultiLocation = MultiLocation::Null;
...
///
/// Ours is only aware of the Balances pallet, which is mapped to `AAALocation`.
pub type LocalAssetTransactor =
	XcmCurrencyAdapter<
		// Use this currency:
		Balances,
		// Use this currency when it is a fungible asset matching the given location or name:
		IsConcrete<AaaLocation>,
...
impl xcm_executor::Config for XcmConfig {
	...
	type Trader = UsingComponents<WeightToFee, AaaLocation, AccountId, Balances, ToAuthor<Runtime>>;
